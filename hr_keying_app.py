    ...
    st.markdown(f"""
        ...
        <div class="sb-step">
            ...
            <div>
                ...
                <div class="sb-step-sub">รองรับ Excel / รูปภาพ</div>
            </div>
        </div>
    """, unsafe_allow_html=True)
...
st.markdown("""
    <div class="main-header">
        ...
        <p>อัปโหลดหลักฐาน ให้ AI คัดแยกหมวดหมู่ และจับคู่กรอกข้อมูลพนักงานลงเทมเพลตให้อัตโนมัติ</p>
    </div>
""", unsafe_allow_html=True)
...
if templates_available:
    ...
    if os.path.exists(t_download_path):
        with open(t_download_path, "rb") as f:
            with col_download:
                st.download_button(label="📥 ดาวน์โหลดเทมเพลตเปล่า", data=f, file_name=selected_t_download, use_container_width=True)
else:
...
uploaded_file = st.file_uploader("ลากและวางไฟล์หรือรูปภาพตรงนี้", type=["xlsx", "xls", "png", "jpg", "jpeg"], label_visibility="collapsed")
if uploaded_file is not None:
    ...
    if is_image:
        ...
        st.image(image_obj, caption="รูปหลักฐานที่นำเข้าสู่ระบบ", width=300)
st.markdown('</div>', unsafe_allow_html=True)
        ...
        with st.spinner("🤖 AI กำลังอ่านข้อมูลและประมวลผล กรุณารอสักครู่..."):
            try:
                ...
                if category == "PERSONAL_DATA":
                    st.markdown('<div class="badge-personal">🆔 คัดแยกสำเร็จ: หมวดหมู่ข้อมูลส่วนตัว / บัตรประชาชน</div>', unsafe_allow_html=True)
                elif category == "COMMISSION":
                    st.markdown('<div class="badge-commission">💰 คัดแยกสำเร็จ: หมวดหมู่รายได้ / ค่าคอมมิชชั่น</div>', unsafe_allow_html=True)
                else:
                    st.markdown('<div class="badge-ot">⏱️ คัดแยกสำเร็จ: หมวดหมู่เวลาทำงาน / โอที (OT)</div>', unsafe_allow_html=True)
                ...
                for item in updates:
                    ...
                    display_name = existing_name if existing_name else "พนักงานรายใหม่"
                ...
                if row_preview_list:
                    ...
                    out_stream.seek(0)
                    st.download_button(label="📥 ดาวน์โหลดไฟล์ Excel ผลลัพธ์สมบูรณ์", data=out_stream, file_name=f"HR_SUCCESS_{template_name}", use_container_width=True)
            except Exception as e:
...
with st.expander("⚙️ สำหรับผู้ดูแลระบบ: อัปโหลดเทมเพลตใหม่"):
    ...
    if new_template_file is not None:
        with open(os.path.join(TEMPLATES_FOLDER, new_template_file.name), "wb") as f:
