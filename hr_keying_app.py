...
TEXT = {
    "th": {
        ...
        "admin_title": "⚙️ สำหรับผู้ดูแลระบบ: อัปโหลดเทมเพลตแม่แบบใหม่",
        "admin_upload_label": "อัปโหลดไฟล์แม่แบบ Excel (.xlsx)",
        "admin_upload_success": "บันทึกเทมเพลตเรียบร้อย!"
    },
    "en": {
        "page_title": "HR Smart Matrix Hub",
        "sidebar_logo_text": "HR Matrix Hub",
        ...
        "admin_title": "⚙️ For Admin: Upload New Template",
        "admin_upload_label": "Upload Excel Template (.xlsx)",
        "admin_upload_success": "Template saved successfully!"
    },
}
...
st.markdown("""
    ...
    [data-testid="stAppViewContainer"] { background-color: #0D0D14; }
    [data-testid="stHeader"] { background-color: rgba(0,0,0,0); }
    [data-testid="stSidebar"] { background-color: #11111A; border-right: 1px solid #23232F; }
    ...
    .step-pill { display: flex; align-items: center; gap: 8px; background: #15151F; border: 1px solid #23232F; border-radius: 999px; padding: 8px 18px 8px 10px; font-size: 0.86rem; font-weight: 600; color: #C9C2FF; }
    ...
    .badge-personal { background-color: rgba(248, 113, 113, 0.1); color: #FCA5A5; border: 1px solid rgba(248, 113, 113, 0.3); padding: 10px 20px; border-radius: 10px; font-weight: 600; }
    .badge-commission { background-color: rgba(74, 222, 128, 0.1); color: #86EFAC; border: 1px solid rgba(74, 222, 128, 0.3); padding: 10px 20px; border-radius: 10px; font-weight: 600; }
    .badge-ot { background-color: rgba(251, 191, 36, 0.1); color: #FCD34D; border: 1px solid rgba(251, 191, 36, 0.3); padding: 10px 20px; border-radius: 10px; font-weight: 600; }
    .badge-generic { background-color: rgba(109, 93, 251, 0.12); color: #C9C2FF; border: 1px solid rgba(109, 93, 251, 0.35); padding: 10px 20px; border-radius: 10px; font-weight: 600; }
    div.stButton > button:first-child { background: linear-gradient(135deg, #6D5DFB 0%, #8B7CFF 100%) !important; color: white !important; border-radius: 10px !important; font-size: 16px !important; font-weight: 600 !important; padding: 14px 30px !important; border: none !important; width: 100% !important; box-shadow: 0 4px 16px rgba(109, 93, 251, 0.3) !important; }
    div.stDownloadButton > button:first-child { background: #1A1A26 !important; color: #C9C2FF !important; border: 1.5px solid rgba(109, 93, 251, 0.4) !important; border-radius: 10px !important; font-weight: 600 !important; padding: 12px 24px !important; width: 100% !important; }
    ...
    h1, h2, h3, h4, h5, p, span, label { color: #E4E5EC; }
    .helper-text { font-size: 0.85rem; color: #777A8C; margin-top: 6px; }
    .divider-line { border-top: 1px dashed #23232F; margin: 22px 0; }
    </style>
""", unsafe_allow_html=True)

# เรียกใช้งาน API Key จากระบบหลังบ้าน
try:
    GEMINI_API_KEY = st.secrets["GEMINI_API_KEY"]
    client = genai.Client(api_key=GEMINI_API_KEY)
except Exception as e:
    st.error("❌ ไม่พบ API Key ในระบบหลังบ้าน กรุณากรอกใน Advanced Settings")
    st.stop()
...
# 💡 ดึงโครงสร้างเทมเพลตแบบ Live Check เพื่อป้องกันแคชค้างจากฝั่ง Cloud หลังบ้าน
...
with st.sidebar:
    ...
    st.markdown(f"""
        <div class="sb-logo">
            <div class="sb-logo-icon"><img src="data:image/png;base64,{LOGO_BASE64}"/></div>
    ...
    st.markdown(f"""
        ...
        <div class="sb-step">
            ...
            <div>
                ...
                <div class="sb-step-sub">{T['sidebar_step2_sub_pending']}</div>
            </div>
        </div>
    """, unsafe_allow_html=True)
...
if templates_available:
    ...
    if os.path.exists(t_download_path):
        with open(t_download_path, "rb") as f:
            with col_download:
                st.download_button(label=T["step1_download_btn"], data=f, file_name=selected_t_download, use_container_width=True)
else:
...
uploaded_file = st.file_uploader(T["step2_uploader_label"], type=["xlsx", "xls", "png", "jpg", "jpeg"], label_visibility="collapsed")
if uploaded_file is not None:
    ...
    if is_image:
        ...
        st.image(image_obj, caption=T["step2_image_caption"], width=300)
st.markdown('</div>', unsafe_allow_html=True)
    ...
    if st.button(T["step3_analyze_btn"], use_container_width=True):
        with st.spinner(T["step3_spinner"]):
            try:
                ...
                if is_image:
                    ...
                    for sheet_name in wb_in.sheetnames:
                        ...
                        for row in ws_in.iter_rows(values_only=True):
                            ...
                            if row_clean: input_text += " | ".join(row_clean) + "\n"
                ...
                โครงสร้างเทมเพลต: {blueprint}
                ตอบกลับเป็นรูปแบบ JSON โครงสร้างแบบนี้เท่านั้น ห้ามมีคำอธิบายอื่น:
                {{
                  "category": "PERSONAL_DATA" หรือ "COMMISSION" หรือ "OVERTIME",
                  "selected_template": "ชื่อไฟล์เทมเพลตปลายทาง.xlsx",
                  "updates": [ {{"row_index": 2, "data_to_fill": {{"ชื่อคอลัมน์": "ค่าที่ได้"}}}} ]
                }}
                """
                ...
                for item in updates:
                    ...
                    display_name = existing_name if existing_name else T["new_employee_label"]
                ...
                if row_preview_list:
                    st.balloons()
                    st.success(T["success_msg"].format(template_name=template_name))
                    ...
                    out_stream.seek(0)
                    st.download_button(label=T["download_result_btn"], data=out_stream, file_name=f"HR_SUCCESS_{template_name}", use_container_width=True)
            except Exception as e:
                st.error(f"{T['error_processing']}{e}")
st.markdown('</div>', unsafe_allow_html=True)

# ===================== 🎯 ส่วนที่แก้ไขเพิ่มเติม (MANAGEMENT): เพิ่มระบบล้างแคชและ Rerun อัตโนมัติ =====================
with st.expander(T["admin_section_title"]):
    new_template_file = st.file_uploader(T["admin_upload_label"], type=["xlsx"])
    if new_template_file is not None:
        ...
        # ล้างหน่วยความจำแคชเก่าหลังบ้าน และสั่งให้สแกนหาไฟล์เทมเพลตใหม่ในโฟลเดอร์ทันที
        st.session_state["templates_available"] = learn_templates()
        st.success(T["admin_upload_success"].format(name=new_template_file.name))
        st.rerun()  # 💡 บังคับให้แอปพลิเคชันรีเฟรชหน้าจออัปเดตข้อมูล Real-time
